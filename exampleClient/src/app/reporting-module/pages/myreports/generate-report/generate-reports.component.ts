...
export class GenerateReportComponent implements OnInit, OnDestroy {
  ...
  getMeasure(m: any, index: any) {
    ...
    if (this.queryParam.measures[index]) {
      ...
      this.measuresChipArray[index].selectedAggregation = m
      const defaultTimeDimension = this.reOrderMeta[metakey].dimensions.filter((x:any) => x.type === 'time').map((y:any) => y.name);
  ...
  removeTimeFilter(i: any) {
    this.queryParam.timeDimensions.splice(i, 1);
    this.buildQuery();
  }

  getTimeFilterFor(ff: any) {
  ...
  selectOperator(i: any, e: any, dim: any) {
  ...
  addFilterItem(i: any, v: any, type: any) {
    if (type === 'm') {
  ...
  removeFilterVal(index1: any, index2: any) {
